...
function MB_one_service($POST){
	...
	$display_one_service = get_post_meta($POST->ID, 'display_one_service', true);
    ...
    ?>
        ...
        <div class="head-metabox" style="border-bottom: 1px solid #ddd; margin-bottom: 1.5rem;">
			<div style="line-height: 5;">
	            <label for="display_one_service" style="margin-right: 15px; font-weight: bold;">Afficher le service : </label>
				<span style="margin-right: 15px;"><input type="radio" <?php checked($display_one_service, 'oui'); ?> name="display_one_service" value="oui"/>Oui</span>
				<span style="margin-right: 15px;"><input type="radio" <?php checked($display_one_service, 'non'); ?> name="display_one_service" value="non"/>Non</span>
		    </div>

            <div  style="margin-bottom: 2rem;">
				<label for="titre_one_service" style="margin-right: 1rem; font-weight: bold;">Titre </label>
	            <input type="text" id="titre_one_service" name="titre_one_service" value="<?php echo $titre_one_service ?>" style="width: 90%;" />
            </div>
        </div>
...
function save_MB_one_service($POST_ID){
	...
	// Title : titre_one service
	if(isset($_POST['titre_one_service'])){
        update_post_meta($POST_ID, 'titre_one_service', $_POST['titre_one_service']);
    }

	// Radio : affiche_one_servie
	if(isset($_POST['display_one_service'])){
        update_post_meta($POST_ID, 'display_one_service', $_POST['display_one_service']);
    }


	// repeatable : repeat_one_service
